...
const insertArticle = (author, title, body, topic) => {
  ...
  return Promise.all(promises).then((results) => {
    return db
      .query(
        `INSERT INTO articles (author, title, body, topic, votes, created_at) VALUES ($1,$2,$3,$4, $5, current_timestamp) RETURNING *`,
        values
      )
      .then(({ rows: articles }) => {
        return articles[0];
...
const deleteArticleById = (article_id) => {
  return deleteCommentsByArticleId(article_id).then(() => {
    return db
